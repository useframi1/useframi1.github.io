...
function displayUnis(unis) {
    for (let i = 0; i < unis.length; i++) {
        ...
        cardDiv.className = "card border-light mb-3";
        ...
        titleLink.className =
            "link-offset-2 link-dark link-underline link-underline-opacity-0 flex-grow-1";

        // Create the title
        let title = document.createElement("h5");
        title.className = "card-title uni-name";
        ...
        document.getElementById("FavoriteUnis").appendChild(mainDiv);
    }
}
...
let btnGoToApplication = document.getElementById("btnGoToApplication");
btnGoToApplication.addEventListener("click", function (event) {
    if (event.target) {
